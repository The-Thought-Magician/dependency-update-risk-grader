  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-neutral-100">Incident Replay Library</h1>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <input
          ...
          className="min-w-[16rem] flex-1 rounded-lg border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm text-neutral-200 placeholder:text-neutral-600 focus:border-lime-500 focus:outline-none"
        />
        <select
          value={ecosystem}
              ...
              <CardHeader className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="truncate text-sm font-semibold text-neutral-100">{inc.name}</h3>
                    {inc.year != null && <span className="text-xs text-neutral-600">{inc.year}</span>}
                  </div>
                ...
                <div className="flex flex-col items-end gap-1">
                  <span className="text-[10px] uppercase tracking-wide text-neutral-600">Expected</span>
              ...
              <CardBody className="flex flex-1 flex-col gap-3">
                {(inc.from_version || inc.to_version) && (
                  <div className="flex items-center gap-2 text-xs text-neutral-400">
                    <code className="rounded bg-neutral-800 px-1.5 py-0.5">{inc.from_version || '—'}</code>
                    <span className="text-neutral-600">→</span>
                    <code className="rounded bg-neutral-800 px-1.5 py-0.5 text-amber-300">{inc.to_version || '—'}</code>
                  </div>
                )}
                {inc.summary && <p className="line-clamp-3 text-sm text-neutral-400">{inc.summary}</p>}
                {inc.catching_factor && (
                  <div className="text-xs text-neutral-500">
                    Caught by{' '}
                    <span className="font-medium text-lime-300">{inc.catching_factor.replace(/_/g, ' ')}</span>
                  </div>
                )}
        ...
        {detailLoading ? (
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="rounded-lg border border-neutral-800 bg-neutral-950 px-3 py-2">
              <div className="text-xs uppercase tracking-wide text-neutral-600">Package</div>
              <div className="mt-1 flex items-center gap-2 text-neutral-200">
                <code className="rounded bg-neutral-800 px-1.5 py-0.5">{detail.package_name}</code>
                <span className="text-xs text-neutral-500">
                  {detail.from_version || '—'} → <span className="text-amber-300">{detail.to_version || '—'}</span>
                </span>
              </div>
            </div>
            {detail.summary && <p className="text-neutral-400">{detail.summary}</p>}
            {detail.catching_factor && (
              <div className="text-neutral-400">
                Catching factor:{' '}
                <span className="font-medium text-lime-300">{detail.catching_factor.replace(/_/g, ' ')}</span>
              </div>
            )}
            {detail.details && Object.keys(detail.details).length > 0 && (
              <pre className="max-h-56 overflow-auto rounded-lg border border-neutral-800 bg-neutral-950 p-3 text-xs text-neutral-400">
      ...
      >
        {replayResult && (
          <div className="space-y-4 text-sm">
            ...
            <div className="flex items-center justify-around rounded-lg border border-neutral-800 bg-neutral-950 px-4 py-4">
              <div className="text-center">
                <div className="text-xs uppercase tracking-wide text-neutral-600">Expected</div>
                <div className="mt-1">
              ...
              <div className="text-center">
                <div className="text-xs uppercase tracking-wide text-neutral-600">Graded</div>
                <div className="mt-1">
